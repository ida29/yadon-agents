...
- `commands.py` — サブコマンド実装関数（`cmd_start()`, `cmd_stop()`, `send_task()` Python API等）
...
**ヤドン1〜4の個性設定（worker_messages の4カテゴリ）:**
...
ヤドンは作業中、思考を常に吹き出しで口に出す（独り言）。例：ファイル読み取り時「ふむふむ よんでる」、コード記述時「かきかき」、考え中「んー」、発見時「あ みつけた」、迷い時「うーん」、進捗「あと ちょっと」。短い独り言（2-6文字程度）を頻繁に出し、各ヤドンの個性（やぁん/ふぁぁ/おなかすいた/えへへ）を混ぜる。
...
**注記:** `yadon send` 公開コマンドは廃止されました。Python API の `send_task()` 関数（`commands.py`）を使用してください。CLIからの直接送信は内部用 `yadon _send` コマンドのみです。
