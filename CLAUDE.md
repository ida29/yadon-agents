...
│       │   └── yadon/               # ヤドン標準テーマ
│       │       ├── __init__.py      # YadonTheme定義（吹き出しメッセージ・スプライト色設定）
...
| ヤドン | 特徴 | 語尾 | メッセージトーン | スプライト色 |
|------|------|------|----------------|----------|
| **ヤドン1** | 素直で元気 | 「やぁん」 | シンプル・前向き<br/>「やるやぁん」「できたやぁん」「まかせて」 | 黄色（通常）/ ピンク（Shiny） |
| **ヤドン2** | ねむてて怠い | 「ふぁぁ」 | ゆるい・眠そう<br/>「ふぁぁ やるね」「ねむい」「zzz」 | 黄色（通常）/ ピンク（Shiny） |
| **ヤドン3** | 食いしん坊 | なし（素） | 食べ物中心<br/>「おなかすいたけど やる」「ごはん たべよう」 | 黄色（通常）/ ゴールド（Galarian） |
| **ヤドン4** | 明るく優しい | 「えへへ」 | 笑顔・楽しい<br/>「えへへ」「たのしいね」「ぴょこぴょこ」 | 黄色（通常）/ オレンジ（Galarian Shiny） |

**マネージャー（ヤドラン）の役割メッセージ:**
- ワーカー配分時: 「タスク きた みんな おねがい」（子分たちへの呼びかけ）
- フェーズ移行時: 「みんな がんばって」「つぎは これ おねがい」（励まし・指示）
- 成功時: 「みんな がんばった」「しっぽが うれしそう」（褒める・喜ぶ）
- 失敗時: 「ちょっと うまく いかなかった」「もういっかい やってみよう」（慰め・再チャレンジ）
...
**変更理由:**
1. **直接性** — 「...」による間延びを削減し、メッセージが即座に伝わる
2. **素朴さ強調** — 装飾的な前置きを避けることで、シンプルで温もりのある印象を確保
3. **テンション維持** — 長く使うUIでも疲れさせない軽快さを実現
4. **スクリーンスペース効率** — 吹き出し幅を固定（40文字）しているため、「...」を削減することで有効テキスト領域を確保

**個別メッセージと共通メッセージの統一:**

- **ワーカー個別メッセージ** (`worker_messages`): 各ヤドン1〜4 の `task`, `success`, `error`, `random` をそれぞれ辞書で管理。ヤドンごとに異なるキャラクターを表現
- **共通ウェルカムメッセージ** (`welcome_messages`): ヤドン全体で使うランダムメッセージ。起動時に各ペットが表示
- **マネージャー専用メッセージ** (`manager_messages`, `manager_phase_messages`, `manager_success_messages`, `manager_error_messages`): ヤドラン固有の役割を果たすメッセージセット
