...
class YadonPet(BasePet):
    ...
    def __init__(self, yadon_number: int, variant: str = 'normal'):
        ...
        messages = YADON_MESSAGES.get(yadon_number, []) + RANDOM_MESSAGES
...
def main() -> None:
    ...
    parser.add_argument('--number', type=int, required=True, help='ヤドン番号 (1-4)')
    parser.add_argument('--variant', default=None, help='カラーバリアント')
    args = parser.parse_args()

    variant = args.variant or YADON_VARIANTS.get(args.number, 'normal')
    ...
    log_debug("yadon_pet", f"Started ヤドン{args.number} variant={variant} pos=({x_pos},{y_pos})")
